...
<div class="userMD_all">
  ...
  <div class="userMD_divheader3">
        ...
        <div class="new_tab">
        	...
        	<a onclick="fun2()">安装订单</a>
        	<a onclick="fun3()">保养订单</a>
        	<a  class="on"  onclick="fun4()">维修订单</a>
        </div>
   ...
   <c:if test='${not empty "${repair_list}"}'>
  			<c:forEach  items="${repair_list}" var="data" varStatus="s">
            <tr class="userMD_TableTrText">
                <td>${s.index+1}</td> 
                <td>${data.repair_order_id}</td> 
                ...
                <td><span class="userMD_activespan2">维修</span></td>
                <td>${data.time}</td>
      ...
      <c:if test='${not empty "${repair_list}"}'>
     	<div class="userMD_divfooter">
	        <div class="userMD_page">
	            <a href="targetUserMangementDetailRepair.do?au_id=${repair_list[0].au_id}&pageNumber=${min}"><input type="button" value="上一页" /></a><!-- ../backageController/ -->
	            <span>当前第${middle}页,共${page}页</span>
	            <a href="targetUserMangementDetailRepair.do?au_id=${repair_list[0].au_id}&pageNumber=${num}"><input type="button" value="下一页"/></a>
	        </div>
   		</div>
      </c:if>	
</div>
