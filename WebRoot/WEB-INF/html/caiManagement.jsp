...
<div class="caiM_all">
    <div class="caiM_divheader">
        <div class="caiM_divheader_div1">
            ...
            <input type="button" onclick="fun1()" value="未完成"/>
    ...
    <div class="caiM_TableOut">
        <table class="caiM_Table">
           ...
           <c:if test='${not empty "${list}"}'>
            ...
            <tr class="caiM_TableTrText">
                ...
                <td><input type="button" value="未支付" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==1}">
                <td><input type="button" value="已支付" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==2}">
                <td><input type="button" value="待收货" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==3}">
                <td><input type="button" value="已收货" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==4}"><!--  0未支付1已支付,2待收货3已收货4安装中5保养中6维修中7已评价8预生成 -->
                <td><input type="button" value="安装中" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==5}">
                <td><input type="button" value="保养中" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==6}">
                <td><input type="button" value="维修中" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==7}">
                <td><input type="button" value="已评价" class="caiM_TableTrTextSure"></td>
                </c:if>
                <c:if test="${data.order_state==8}">
                <td><input type="button" value="预生成订单" class="caiM_TableTrTextSure"></td>
                </c:if>
            </tr>
            </c:forEach>
            </c:if>
        ...
        <div class="caiM_Footer">
            <div class="caiM_FooterPage">
                <a href="../backageController/caiManagement.do?pageNumber=${min}"><input type="button" value="上一页"/></a>
                <span>当前第${middle}页,共${page}页</span>
                <a href="../backageController/caiManagement.do?pageNumber=${num}"><input type="button" value="下一页"/></a>
            </div>
        </div>
        </c:if>
    </div>
...
function fun1(){
	window.location.href="../backageController/commodity_orderN.do";
}
function fun2(){
